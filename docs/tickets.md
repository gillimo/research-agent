...
- [x] F1: Embedding/vector choice + config  
  Defaults set to `bge-small-en-v1.5` + FAISS in `config/local.yaml` with rationale in docs.  
  Deps: none.
...
- [x] F3: Test harness + fixtures  
  Pytest skeleton; tests for sanitization/allowlist, schema validation, FAISS ingest/retrieve, CLI args/stdin.  
  Deps: F1, F2.

P2 — Core local loop
- [~] C1: `researcher` CLI skeleton  
  Commands: `ask`, `ingest`, `status`; stdin piping; provenance/confidence in output.  
  Deps: F1, F2.
- [~] C2: Ingestion + RAG plumbing  
  Loader + chunker; embed/index writer; retrieval with scored chunks and sources.  
  Deps: F1.
- [~] C3: Observability (local)  
  Standard log format, provenance tags, rotation/size limits.  
  Deps: C1, C2.

P3 — Martin UX parity (local-first)
- [~] M1: Port Martin behaviors to `researcher`  
  Command-plan extraction, smart runner (interactive vs non-interactive), diagnosis loop, progress summaries, rephraser toggle; pipe-friendly.  
  Deps: C1, C2, F2.
- [ ] M2: Internal abilities  
  ...
  Deps: M1.
- [~] M3: Agent oversight/prompting  
  Supervisor loop to monitor coding agents and nudge/resume when idle, following documented guardrails; configurable prompts.  
  Deps: M1, C1.
